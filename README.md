...
Upload: Receive file → save → record in DB
Queue: Add task to Celery
OCR/Parse:
PDF: PyMuPDF/pdfplumber (text/tables) → fallback OCR
Images: PaddleOCR → field extraction
Normalize: Dates, currencies, amounts
Categorize: Rules + ML model
Save: Transactions + line_items to DB
Testing
# Run all tests
pytest
...
Contact
Questions and suggestions: GitHub Issues
# Personal Finance Tracker
